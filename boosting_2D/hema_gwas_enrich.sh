...
do
  ...
  touch $pval_file
  for result in $(ls $RANK_PATH*"/"*"hema_"$comp"_up_test_region_overlaps.txt");
  do
    gwas0=$(basename $result)
    var="_hema_"$comp"_test_region_overlaps.txt"
    gwas=${gwas0/$var/}
  ...
  touch $pval_file
  for result in $(ls $RANK_PATH*"/"*"hema_"$comp"_down_test_region_overlaps.txt");
  do
    gwas0=$(basename $result)
    var="_hema_"$comp"_test_region_overlaps.txt"
    gwas=${gwas0/$var/}

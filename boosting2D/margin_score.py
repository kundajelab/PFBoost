...
def calc_margin_score_x1(tree, y, x1, x2, index_mat, 
    ...
    x1_bundle_string = '|'.join(unique_bundle_terms) if \
                           len(unique_bundle_terms) > 0 else "None"

    # All rules where the motif is above it, used in the split, or bundled in the split
    rules_w_x1_feat = [el for el in xrange(1, tree.nsplit)
    ...
    if len(rules_w_x1_feat) > 0: # can calculate rule fraction only when feature is used
        ...
        if type(index_mat) != type(rule_index_mat):
            raise ValueError("index matrix and rule matrix are not the same type")
    ...
    return [x1_feat_name, x1_bundle_string, margin_score,
            margin_score_norm, rule_index_fraction]

def calc_margin_score_x1_worker(tree, y, x1, x2, index_mat, by_example, 
    ...
    while True:
        ...
        if by_example==True:  
            ...
            with lock:
                ...
                # reshapes view of data, not true data object
                new_matrix = np.frombuffer(matrix_or_list.get_obj()
                                          ).reshape((new_x, new_y))
...
def calc_margin_score_x2(tree, y, x1, x2, index_mat, 
                         x2_feat_index, by_example=False):
    ...
    unique_bundle_terms = np.unique([x2.col_labels[el] 
                                    ...
                                    for el in 
                                    [tree.split_x2[node]] + tree.bundle_x2[node] 
                                    if el != x2_feat_index]).tolist()
    x2_bundle_string = '|'.join(unique_bundle_terms) \
                       if len(unique_bundle_terms)>0 else "None"
    ...
    return [x2_feat_name, x2_bundle_string, margin_score, 
            margin_score_norm, rule_index_fraction]
...
def calc_margin_score_x2_worker(tree, y, x1, x2, index_mat, by_example, 
                                (lock, index_cntr, matrix_or_list)):
...
def calc_margin_score_rule(tree, y, x1, x2, index_mat, x1_feat_index, 
                           x2_feat_index, by_example=False):
    # Feature names
    x1_feat_name = x1.row_labels[x1_feat_index]
    x2_feat_name = x2.col_labels[x2_feat_index]
    ...
    ### WRITE out bundles
    x1_unique_bundle_terms = np.unique([x1.row_labels[el] 
                                        for node in pair_rules 
                                        for el in 
                                        [tree.split_x1[node]] + tree.bundle_x1[node] 
                                        if el != x1_feat_index]).tolist()
    x1_bundle_string = '|'.join(x1_unique_bundle_terms) \
                       if len(x1_unique_bundle_terms)>0 else "None"
    x2_unique_bundle_terms = np.unique([x2.col_labels[el] for node in pair_rules 
                                        for el in [tree.split_x2[node]]
          +tree.bundle_x2[node] if el != x2_feat_index]).tolist()
    x2_bundle_string = '|'.join(x2_unique_bundle_terms) \
                       if len(x2_unique_bundle_terms)>0 else "None"
    ...
    ### but any node that is it as first split or bundle)
    ...
    if len(rules_w_x1_feat_and_x2_feat)>0:
        ...
        rule_index_fraction=0
    # print rule_index_fraction
    return [x1_feat_name, x1_bundle_string, x2_feat_name, x2_bundle_string, 
            margin_score, margin_score_norm, rule_index_fraction]
...
def calc_margin_score_node(tree, y, x1, x2, index_mat, node, by_example=True):
    ...
    x1_bundle_string = '|'.join([x1.row_labels[el] 
                                 for el in tree.bundle_x1[node]])
    x2_bundle_string = '|'.join([x2.col_labels[el] 
                                 for el in tree.bundle_x2[node]])
    ...
    # print rule_index_fraction
    return [node, x1_feat_name, x1_bundle_string, x2_feat_name, 
            x2_bundle_string, margin_score, margin_score_norm, 
            rule_index_fraction, direction]
...
def calc_margin_score_path(tree, y, x1, x2, index_mat, node, by_example=False):
    ...
    # Get Prediction Matrix (remove score of all nodes in path for 
    # just the index of rules that get to end of path)
    pred_adj = tree.pred_train
    ...
    path_string='{0}_{1}'.format(node,
        '|'.join([','.join([x1.row_labels[tree.split_x1[n]], 
                            x2.col_labels[tree.split_x2[n]]]) 
                            for n in nodes_in_path]))

    # print rule_index_fraction
    return [node, path_string, len(nodes_in_path), margin_score, 
            margin_score_norm, rule_index_fraction, direction]
...
def rank_by_margin_score(tree, y, x1, x2, index_mat, method):
    # config.NCPU=1
    ...
    if method == 'x1':
        ...
        # Run in parallel
            ...
            args = [tree, y, x1, x2, index_mat, False, 
        ...
        ranked_score_df = pd.DataFrame(
            {'x1_feat':[el[0] for el in rule_processes], \
    ...
    if method == 'x2':
        ...
        used_x2_feats = np.unique([el for el in tree.split_x2 
                                   if el != 'root'] + \
        ...
        # Report data frame with feature 
        ranked_score_df = pd.DataFrame(
            {'x2_feat':[el[0] for el in rule_processes], \
    ...
    if method == 'x1_and_x2':
        ...
        for (x1_feat, x2_feat) in uniq_x1_x2_pairs.values():
            result = calc_margin_score_rule(tree, y, x1, x2, index_mat, 
                                            x1_feat, x2_feat)
            rule_processes.append(result)
        # Report data frame with feature 
        ranked_score_df = pd.DataFrame(
    ...
    if method == 'node':
        ...
        # Report data frame with feature 
        ranked_score_df = pd.DataFrame(
            {'node':[el[0] for el in rule_processes], \
    ...
    if method == 'path':
        ...
        # Run in parallel
            ...
            lock = multiprocessing.Lock()
            manager = multiprocessing.Manager()
            rule_processes_mp = manager.list()
            index_cntr = multiprocessing.Value('i', 1)
            args = [tree, y, x1, x2, index_mat, False, 
                    (lock, index_cntr, rule_processes_mp)]
...
def split_index_mat_prom_enh(index_mat, y, tss_file):
    ...
    temp_bed_file = os.path.splitext(y.data_file)[0] + \
                    '_row_coords_TEMP.txt'
    temp_intersect_file = os.path.splitext(y.data_file)[0] + \
                          '_row_coords_tss_intersect_TEMP.txt'
    row_coords=[el.split(';')[0].replace(':', '\t').replace('-','\t') 
                for el in y.row_labels]
...
def call_rank_by_margin_score(index_mat, key, method, prefix, y, x1, x2, 
                              tree, num_perm=100, null_tree_file=None):
    ...
    rank_score_df = rank_by_margin_score(tree, y, x1, x2, index_mat,
                                         method=method)
    ...
    if num_perm > 0:
        ...
        # Compute p-value by shuffling y values
            rank_score_df = calculate_null_margin_score_dist_by_shuffling_target(
                                rank_score_df,  index_mat, method, num_perm, 
...
def calculate_null_margin_score_dist_by_shuffling_target(rank_score_df, 
    ...
    if method=='x1':
        ...
        for i in xrange(num_perm):
            ...
            margin_score_dict['perm{0}'.format(i)] = \
                rank_by_margin_score(tree, y_null, x1, x2, new_index, 
                                     method=method)
    elif method=='x2':
        # For each permutation calculate margin scores and add to dictionary
        for i in xrange(num_perm):
        ...
        raise ValueError("provide method in [x1, x2, x1_and_x2, node, path]")
...
def calculate_null_margin_score_dist_from_NULL_tree(rank_score_df, index_mat, 
    ...
    if type(tree_null.pred_train) != type(y.data):
        print 'NULL and true models are incompatible types. Use models of same sparse/dense type.'
        return 1
    ...
    if method=='x1':
        ...
        for i in xrange(num_perm):
            ...
            margin_score_dict['perm{0}'.format(i)]=rank_by_margin_score(tree_null,
             y_null, x1, x2, new_index, method=method)
    elif method=='x2':
        # For each permutation calculate margin scores and add to dictionary
        for i in xrange(num_perm):
            # Permute columns
...
def plot_norm_margin_score_across_conditions(conditions, method, plot_label, 
                                             num_feat, element_direction):
...
def call_discriminate_margin_score(index_mat_1, index_mat_2, key, method, 
    ...
    rank_score_df_1 = rank_by_margin_score(tree, y, x1, x2, 
                                           index_mat_1, method=method)
    ### calculate margin score with second set
    rank_score_df_2 = rank_by_margin_score(tree, y, x1, x2, 
